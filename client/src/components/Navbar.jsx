import React, { useState, useEffect } from 'react';
import Cimg from '../assets/Rectangle 14.png';
import { useNavigate, useLocation } from 'react-router-dom';
import { BookOpen, Briefcase, ClipboardCheck, Users } from 'lucide-react';

const Navbar = () => {
  const navigate = useNavigate();
  const location = useLocation();

  // Assume `isAuthenticated` is retrieved from a global state, context, or API
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  const [userProfile, setUserProfile] = useState(null); // Store user profile data like name, image

  useEffect(() => {
    // Simulate fetching user data to check login status
    const fetchUserData = () => {
      const token = localStorage.getItem('token'); // Check if token exists
      if (token) {
        // Mock user data - replace with actual API call
        setIsAuthenticated(true);
        setUserProfile({
          name: 'John Doe',
          profilePicture: 'data:image/jpeg;base64,', // Replace with actual image URL
        });
      } else {
        setIsAuthenticated(false);
        setUserProfile(null);
      }
    };

    fetchUserData();
  }, []);

  const handleLogout = () => {
    localStorage.removeItem('token'); // Remove token on logout
    setIsAuthenticated(false);
    setUserProfile(null);
    navigate('/'); // Redirect to home or login page
  };

  const handleLearningClick = () => {
    navigate('/courseCreation');
  };
  const handleFreeLanceClick = () => {
    navigate('/freelance');
  };
  const handleQuizClick = () => {
    navigate('/modules');
  };
  const handleFormClick = () => {
    navigate('/clientForm');
  };

  const getUnderlineStyle = (path) => {
    return location.pathname === path ? 'underline decoration-[#9E00D7] underline-offset-4' : '';
  };

  return (
    <>
      <div className="bg-black h-[15px] w-full" />
      <div className="h-[90px] w-full bg-[#252323] flex items-center">
        {/* Logo Section */}
        <div className="pl-[8px] relative" onClick={() => navigate('/')}>
          <h1 className="text-black absolute ml-[12px] pt-[2px] font-semibold cursor-pointer">collab-o</h1>
          <img src={Cimg} alt="" className="h-[40px] w-[99px] cursor-pointer" />
        </div>

        {/* Navigation Links Section */}
        <div className="flex items-center ml-[400px]">
          {/* Learning Link */}
          <div className="group relative mx-10 flex items-center">
            <h1
              className={`text-[#d6cdcd] text-[20px] cursor-pointer transition-all duration-300 hover:text-white flex items-center ${getUnderlineStyle(
                '/courseCreation'
              )}`}
              onClick={handleLearningClick}
            >
              <BookOpen size={18} className="mr-2" /> Learning
            </h1>
          </div>

          {/* Freelance Link */}
          <div className="group relative mx-10 flex items-center">
            <h1
              className={`text-[#d6cdcd] text-[20px] cursor-pointer transition-all duration-300 hover:text-white flex items-center ${getUnderlineStyle(
                '/freelance'
              )}`}
              onClick={handleFreeLanceClick}
            >
              <Briefcase size={18} className="mr-2" /> Freelance
            </h1>
          </div>

          {/* Take Quiz Link */}
          <div className="group relative mx-10 flex items-center">
            <h1
              className={`text-[#d6cdcd] text-[20px] cursor-pointer transition-all duration-300 hover:text-white flex items-center ${getUnderlineStyle(
                '/modules'
              )}`}
              onClick={handleQuizClick}
            >
              <ClipboardCheck size={18} className="mr-2" /> Take quiz
            </h1>
          </div>

          {/* Hire Link */}
          <div className="group relative mx-10 flex items-center">
            <h1
              className={`text-[#d6cdcd] text-[20px] cursor-pointer transition-all duration-300 hover:text-white flex items-center ${getUnderlineStyle(
                '/clientForm'
              )}`}
              onClick={handleFormClick}
            >
              <Users size={18} className="mr-2" /> Hire
            </h1>
          </div>

          {/* User Authentication Section */}
          <div className="ml-[450px] flex items-center gap-4">
            {!isAuthenticated ? (
              <>
                {/* Login Button */}
                <div
                  className="group relative h-[40px] w-[80px] items-center border-2 border-[#9E00D7] bg-white rounded-[5px] transition-all duration-300 ease-in-out hover:rounded-full cursor-pointer"
                  onClick={() => navigate('/login')}
                >
                  <h1 className="font-semibold ml-[17px] mt-[6px]">Login</h1>
                </div>

                {/* Signup Button */}
                <div
                  className="group relative cursor-pointer text-[#d6cdcd] text-[20px] transition-all duration-300 hover:text-white"
                  onClick={() => navigate('/signup')}
                >
                  Signup
                </div>
              </>
            ) : (
              <div className="relative">
                {/* Profile Picture */}
                <img
                  src={userProfile?.profilePicture}
                  alt="Profile"
                  className="h-[40px] w-[40px] ml-[100px] rounded-full cursor-pointer"
                  onClick={handleLogout}
                />
                <div className="absolute top-[50px] right-0 bg-white text-white  p-2 rounded shadow-lg hidden group-hover:block">
                  <button onClick={handleLogout} className="text-red-500 hover:underline">
                    Logout
                  </button>
                </div>
              </div>
            )}
          </div>
        </div>
      </div>
    </>
  );
};

export default Navbar;